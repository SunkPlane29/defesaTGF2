...
\begin{frame}
	Extraindo dados de massa e raio como segue:
	\begin{equation}
		\begin{aligned}
			M_i          & \sim \mathcal{U}(1,0 \; \text{M}_\odot, M_\text{max}), \quad R_i = R(M_i)                                                                          \\
			M_i^*        & \sim \mathcal{N}(M_i, \sigma_{M,i})                                                                  ,\quad R_i^* = \mathcal{N}(R_i, \sigma_{R,i}) \\
...
\begin{frame}
	...
	\begin{equation}
		\mathcal{L}_\text{BNN} = - \mathbb{E}_{q(\boldmath \theta)} \left[ \log(p(D | \boldmath \theta) \right] + \text{KL}\left( q(\boldmath \theta) \parallel p(\boldmath \theta) \right),
	\end{equation}
...
\begin{frame}{Conclusão e perspectivas}
	Mostramos que:
	\begin{itemize}
		\item A escolha de \textit{likelihoods} Gaussianas não afeta significativamente os resultados;
		\item A parametrização utilizada consegue representar bem equações de estado mais realísticas, apesar de satisfazer causalidade;
		\item A NN consegue inferir as EoSs a partir de dados de massa e raio, mas falha em quantificar incertezas epistêmicas;
		\item O custo computacional de extrair amostras da distribuição posterior via MCMC e \textit{NestedSampling} é alto comparado ao custo de inferência via NN, e aumenta com o número de dados observacionais;
		\item Em um trabalho mais recente \footfullcite{Carvalho_2025}, foi utilizada uma abordagem de \textit{normalizing flows} com redes neurais, da qual o resultado é uma distribuição condicional dos parâmetros, e obteve resultados similares, que a incerteza em $c_s^2$ aumenta com a densidade.
	\end{itemize}
